    ...
    def handle_data(self, text):
        ...
        if len(emojis) != 0:
            ...
            for i in range(Nemo):
                if emojis[i]["match_start"] == 0:
                    ...
                    if i == 0:
                        segments.append((0, emojis[i]["match_end"], False))
                    #add emojus
                    segments.append((emojis[i]["match_start"], emojis[i]["match_end"], True))
                    #add text following emojus
            ...
            for first, last, is_emojus in segments:
                ...
                if is_emojus:
                    # print("emojus", len(text[first:last]), (first, last))
...
def handle_message(
    ...
    if text and not file and validators.url(text if "://" in text else f"https://{text}"):
        ...
        if file:
            ...
            try:
                ...
                raise HTTPException(status_code=415, detail="Uploaded file does not look like an image.")
            else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Receive multi-part forms with text and images from authenticated users, generate a docx-document and cat that to some host.")
